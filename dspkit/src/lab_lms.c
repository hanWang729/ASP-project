...
void my_lms(float const * y, float const * x, float * xhat, float * e, int block_size,
	...
	*/
	 ...
	 *
	 * doing block_size lms update iterationTODOs, i.e. something like:
	 * 
	 * int n;
	...
	for(n=0;n<block_size;n++){
		...
		//int i;
		//for(i = 0;i++;i<=sizeof(lms_coeffs)-1){
		//	//y_book[i] = lms_state[i+n];
		//	float * y_book = &lms_state[i+n];
		//}
		arm_dot_prod_f32(lms_coeffs,y_book,lms_taps,xhat+n);
		e[n] = x[n] - xhat[n];
		float32_t y_tmp[lms_taps];
		arm_scale_f32(y_book,2*lms_mu*e[n],y_tmp,lms_taps);
		arm_add_f32(lms_coeffs,y_tmp,lms_coeffs,lms_taps);
	}
