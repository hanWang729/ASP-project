...
% directory as this file as well as any of the standard matlab functions.
	...
	function z = add_cyclic_prefix(x,Ncp)  %#ok<*INUSD>
        ...
        cp = x(end - Ncp + 1:end);
		z = [cp; x]; %TODO: This line is missing some code!
    end
    ...
    function symb = bits2qpsk(bits)
        ...
        m=transpose(buffer(bits,2));
        symb = (m(:,1) + m(:,2)*1j)/sqrt(2); %TODO: This line is missing some code!
        symb = sqrt(1/2)*(bits(1:2:end)+1*j*bits(2:2:end));
    end
    ...
    function bits  = qpsk2bits(x)
        ...
        a = heaviside(a);
        b = heaviside(b);
        bits(1:2:end) = a; %TODO: This line is missing some code!
        bits(2:2:end) = b; %TODO: This line is missing some code!
...
%         tx_frame_final = real(tx_frame_mod);
        tx_frame_final = tx_frame_mod;
        figure(4);
        plot(abs(fft(tx_frame_final)));
...
%         rx_frame_us = rx_frame_raw;
        figure(6);
        plot(abs(fft(rx_frame_us)));
        
        figure(7);
        fgrid=1/(16000*8):1/(16000*8):1;leng=length(fgrid);
        subplot(2,2,1);
        plot(abs(fft(tx_frame_us)));
        title('upsampled signal');
        subplot(2,2,2);
        plot(abs(fft(tx_frame_final)));
        title("modulated signal(real part)");
        subplot(2,2,3);
        plot(abs(fft(rx_frame_raw)));
        title('received raw signal');
        subplot(2,2,4);
        plot(abs(fft(rx_frame_us)));
        ...
        subplot(2,2,2);
        plot(abs(fft(tx_frame_final)));
        title("modulated signal(whole part)");
        subplot(2,2,3);
        plot(abs(fft(rx_frame_raw)));
        title('received raw signal');
        subplot(2,2,4);
