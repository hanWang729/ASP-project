...
N = 283;         % Number of OFDM (QPSK) symbols to transmit.  68 letters = 4*68 bits 
N_cp = 2;        % Length of cyclic prefix
...
=======
h = 0.8.^(0:59)';            % LP model
>>>>>>> a77d7240f6bdc82b1e8816311f8c06533113170e
...
subplot(2,1,1);
plot(abs(fft(h, N)));
title('Channel response','fontsize',18);
xlabel('k','fontsize',18);
ylabel('|H(k)|','fontsize',18);
subplot(2,1,2);
plot(angle(fft(h, N)));
xlabel('k','fontsize',18);
ylabel('arg(H(k))','fontsize',18);
...
if length(rx) <= 1
    ...
    title('Pre-equalization symbol constellation','fontsize',18);

    figure(3);
    plot_constallation(symbs.tx, symbs.rx_e);
    title('Post-equalization symbol constellation','fontsize',18);
end